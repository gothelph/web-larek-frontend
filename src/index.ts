...
const cardCatalogTemplate = ensureElement<HTMLTemplateElement>(settings.cardSettings.catalogTemplate);
const cardPreviewTemplate = ensureElement<HTMLTemplateElement>(settings.cardSettings.previewTemplate);
const cardBasketTemplate = ensureElement<HTMLTemplateElement>(settings.cardSettings.basketTemplate);
const orderFormTemplate = ensureElement<HTMLTemplateElement>(settings.orderTemplate);
const contactsFormTemplate = ensureElement<HTMLTemplateElement>(settings.contactsTemplate);
...
const contactsForm = new Contacts(eventEmitter, cloneTemplate(contactsFormTemplate));


eventEmitter.on(eventNames.contacts.submit, () => {
  api.processOrder(appState.order).then((result) => {
...
eventEmitter.on(eventNames.contacts.change, (data: { field: keyof IOrderInfo, value: string }) => {
  appState.setOrderPart({ [data.field]: data.value });
});

eventEmitter.on(eventNames.order.change, (data: { field: keyof IOrderInfo, value: string }) => {
  appState.setOrderPart({ [data.field]: data.value });
});
...
eventEmitter.on(eventNames.formErrors.change, (errors: Partial<IOrderInfo>) => {
  ...
  orderForm.errors = Object.values({ paymentMethod, address }).filter(el => !!el).join("; ");
  contactsForm.errors = Object.values({ email, phone }).filter(el => !!el).join("; ");
});
...
eventEmitter.on(eventNames.preview.change, (item: IProductItem) => {
  if (item) {
    ...
    card.button = appState.isInBasket(item.id) ? 'Удалить из корзины' : 'В корзину';
