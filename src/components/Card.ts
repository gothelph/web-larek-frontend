...
export class Card extends Component<IProductItem> {
  ...
  constructor(container: HTMLElement, actions?: ICardActions) {
    ...
    const { category, title, image, price, description, button } = settings.cardSettings.selectors;
  ...
  get id() {
    return getElementData<{ id: string }>(this.container, { id: (data?: string) => data || '' }).id;
  }
  ...
  set category(value: string) {
    ...
    this.container.querySelector(settings.cardSettings.selectors.category)?.classList.add(bem('card', 'category', modifier).name)
  }
