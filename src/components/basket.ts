...
export class BasketView implements IBasketView {
	...
	constructor(events: IEvents, basket: Basket) {
		...
		const basketCounter = basketButton.querySelector(
		...
		events.on<IBasketItem>(EventTypes.ADD_TO_BASKET, (item) =>
			this.addItem(item)
		);
		events.on<Pick<IBasketItem, 'id'>>(
			EventTypes.REMOVE_FROM_BASKET,
	...
	onConfirm() {
		const orderInfoForm = new OrderInfoForm(this.events, this.basket);
