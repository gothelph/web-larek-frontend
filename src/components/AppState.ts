...
export class AppState {
  ...
  validateOrder() {
    ...
    if (!this.order.payment) errors.paymentMethod = 'Необходимо выбрать способ оплаты';

    if (!this.order.address) errors.address = 'Необходимо указать адрес';
    else if (this.order.address.length < 5) errors.address = 'Адрес слишком короткий';

    
    if (!this.order.email) errors.email = 'Необходимо указать email';
    else if (!validateEmail(this.order.email)) errors.email = 'Неверный формат email';
